...
export class QuizService {
    ...
    public async assignQuizAnswers(answers: QuizQuestionAnswerModel[], quizId: string, username: string): Promise<any> {
      ...
      let score = 0
      answers.map((answer) => {
        const question = quiz.questions.filter((question) => question.id === answer.questionId)[0]
        const correctAnswer = question.alternatives.filter((alternative) => alternative.isCorrect === true)[0]
