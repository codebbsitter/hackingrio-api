...
export class EventsService {
  ...
  public async getEventChallengesByEventId(id: string): Promise<EventChallengesEntity[]> {
    const challenges = await this.eventChallengesRepository.getEventChallengesByEventId(id)
    if(!challenges) throw new NotFoundException('There is not active challenges for this event')
    return challenges
  }
